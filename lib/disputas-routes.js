    ...
    if (b.notificar === true && sesEnabled()) {
      try {
        ...
        await (async function () {
          ...
          const campos = [
            ['Folio', caso.folio], ['Marca', caso.brand], ['Canal', caso.channel], ['Ciclo', caso.cycle],
            ['ARN', arn || '—'], ['Case number', case_number || '—'],
            ['Monto', amount != null ? '$' + amount.toFixed(2) + ' ' + (caso.currency_code || '') : '—'],
          ];
          ...
          const { subject, html, textFallback } = armarNotifDisputaHTML({
            ...
            destinatario_nombre: contactos[0].nombre || 'contacto',
            fecha_limite_respuesta: D.fechaISO(fechaLimComercio), campos,
            mensaje_extra: notas,
          }, {});
