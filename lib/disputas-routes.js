...
module.exports = function mountDisputasRoutes(app, deps) {
  ...
  app.get('/api/disputa/chargebacks/xlsx', auth, async (req, res) => {
    ...
    const rows = (await db.query(
      `select cb.*, m.afiliacion as m_afiliacion, m.nombre as m_nombre, m.nombre_cifrado as m_nomcif,
              rc.codigo as rc_codigo, rc.descripcion as rc_desc, rc.categoria as rc_cat
    ...
    const desc = v => { try { return v ? String(C && C.decrypt ? C.decrypt(v.toString('utf8')) : v) : ''; } catch (_e) { return ''; } };
    ...
    let total = 0;
    for (const r of rows) {
      const monto = Number(r.disputed_amount) || 0;
      total += monto;
      ws.addRow([
        r.folio || '',
        r.fecha_evento ? String(r.fecha_evento).slice(0,10) : '',
        r.m_afiliacion || '',
        r.m_nombre || desc(r.m_nomcif) || '',
        r.grupo_cliente || '',
        r.brand || '',
        r.channel || '',
        ...
        r.ticket || '',
        r.auth_code || '',
        desc(r.last4_cifrado) || r.last4 || '',
        r.arn || '',
        r.fecha_cbk ? String(r.fecha_cbk).slice(0,10) : (r.fecha_evento ? String(r.fecha_evento).slice(0,10) : ''),
