...
module.exports = function mountDisputasRoutes(app, deps) {
  ...
  app.get('/api/disputa/chargebacks/xlsx', auth, async (req, res) => {
    ...
    if (f.desde && /^\d{4}-\d{2}-\d{2}$/.test(f.desde)) { vals.push(f.desde); conds.push(`cb.fecha_evento >= $${vals.length}`); }
    if (f.hasta && /^\d{4}-\d{2}-\d{2}$/.test(f.hasta)) { vals.push(f.hasta); conds.push(`cb.fecha_evento <= $${vals.length}`); }
