...
async function aplicarEnCorte(db, corteId, aplicaciones) {
  for (const a of aplicaciones) {
    ...
    // Si (corte_id, cargo_id) ya existía (ej. la misma aplicación se procesó
    // dos veces), el insert no hace nada por el ON CONFLICT — y NO debemos
    // sumar monto_retenido otra vez, o el cargo queda retenido de más sin
    // dejar rastro en la auditoría (bug real detectado: un financiamiento
    // terminó con monto_retenido > monto_original con 0 filas en
    // corte_aplicaciones que lo respaldaran).
