...
function mountTareasRoutes(app, deps) {
  ...
  async function computarReporte(producto, desde, hasta) {
    ...
    // Productividad por producto (matriz completa: rutinas + entregables + SLA + 1a vuelta)
    //   Cuando el filtro `producto` ya restringe uno solo, se devuelve solo ese.
    //   Si no hay filtro de producto, se agrupa por producto de ambas tablas.
    // Se construye por queries separadas para no forzar joins imposibles.
    const wTk2 = [...wTk], vTk2 = [...vTk];
    const wEn2 = [...wEn], vEn2 = [...vEn];
    const rutinasPP = (await db.query(
      `select coalesce(producto,'—') as producto,
              count(*)::int as total,
              coalesce(sum(case when estado='hecho' then 1 else 0 end),0)::int as hechas,
              coalesce(sum(case when estado in ('en_curso','en_revision') then 1 else 0 end),0)::int as en_curso,
              coalesce(sum(case when estado='bloqueado' then 1 else 0 end),0)::int as bloqueadas,
              coalesce(sum(case when estado not in ('hecho') and fecha_limite is not null and fecha_limite < current_date then 1 else 0 end),0)::int as vencidas
         from tareas_tickets`
       + (wTk2.length ? ' where ' + wTk2.join(' and ') : '') +
      ` group by 1`, vTk2
    )).rows;
    const entPP = (await db.query(
      `select coalesce(s.producto,'—') as producto,
              count(*)::int as total,
              coalesce(sum(case when s.estado='aprobada' then 1 else 0 end),0)::int as aprobadas,
              coalesce(sum(case when s.estado='rechazada' then 1 else 0 end),0)::int as rechazadas,
              coalesce(sum(case when s.estado in ('nueva','aceptada','en_ejecucion','requiere_info','validacion') then 1 else 0 end),0)::int as en_curso,
              coalesce(sum(case when s.estado not in ('aprobada','rechazada','cancelada') and s.fecha_limite < current_date then 1 else 0 end),0)::int as vencidas,
              coalesce(sum(case when s.estado='aprobada' and s.fecha_cierre::date <= s.fecha_limite then 1 else 0 end),0)::int as sla_cumplido,
              coalesce(sum(case when s.estado='aprobada' and not exists(
                select 1 from entregables_actividad a
                 where a.solicitud_id = s.id and a.tipo='status_change'
                   and (a.estado_nvo='requiere_info' or a.estado_nvo='rechazada')
              ) then 1 else 0 end),0)::int as primera_vuelta
         from entregables_solicitudes s`
       + (wEn2.length ? ' where ' + wEn2.join(' and ') : '') +
      ` group by 1`, vEn2
    )).rows;
    // Fusión por producto
    const productos = new Set([...rutinasPP.map(x=>x.producto), ...entPP.map(x=>x.producto)]);
    const productividad = [...productos].sort().map(p => {
      const r = rutinasPP.find(x => x.producto === p) || { total:0, hechas:0, en_curso:0, bloqueadas:0, vencidas:0 };
      const e = entPP.find(x => x.producto === p)     || { total:0, aprobadas:0, rechazadas:0, en_curso:0, vencidas:0, sla_cumplido:0, primera_vuelta:0 };
      return {
        producto: p,
        rutinas_total: r.total, rutinas_hechas: r.hechas, rutinas_en_curso: r.en_curso, rutinas_bloqueadas: r.bloqueadas, rutinas_vencidas: r.vencidas,
        rutinas_cumplimiento_pct: r.total > 0 ? Math.round((r.hechas / r.total) * 100) : null,
        entregables_total: e.total, entregables_aprobadas: e.aprobadas, entregables_rechazadas: e.rechazadas,
        entregables_en_curso: e.en_curso, entregables_vencidas: e.vencidas,
        sla_cumplido: e.sla_cumplido,
        sla_pct: e.total > 0 ? Math.round((e.sla_cumplido / e.total) * 100) : null,
        primera_vuelta: e.primera_vuelta,
        primera_vuelta_pct: e.aprobadas > 0 ? Math.round((e.primera_vuelta / e.aprobadas) * 100) : null,
      };
    });
    ...
    // Detalle por entregable con tiempos de transición.
    //   recepcion:  s.creado_at
    //   aceptada:   primer status_change → 'aceptada' o 'en_ejecucion'
    //   cerrado:    s.fecha_cierre (o último status_change a aprobada/rechazada/cancelada)
    const detalleEnt = (await db.query(
      `select s.id, s.folio, s.titulo, s.producto, s.estado, s.prioridad,
              s.fecha_limite::text as fecha_limite,
              s.creado_at, s.fecha_cierre,
              u1.nombre as solicitante_nombre, u2.nombre as ejecutor_nombre,
              t.codigo as tipo_codigo,
              (select min(ts) from entregables_actividad a
                where a.solicitud_id=s.id and a.tipo='status_change'
                  and a.estado_nvo in ('aceptada','en_ejecucion')) as aceptada_at
         from entregables_solicitudes s
       ...
       + (wEn.length ? ' where ' + wEn.join(' and ') : '') +
      ` order by s.creado_at desc
        limit 500`, vEn
    )).rows;

    function calcTiempos(r) {
      const t0 = r.creado_at ? new Date(r.creado_at).getTime() : null;
      const t1 = r.aceptada_at ? new Date(r.aceptada_at).getTime() : null;
      const t2 = r.fecha_cierre ? new Date(r.fecha_cierre).getTime() : null;
      return {
        ms_recepcion_a_aceptacion: (t0 && t1) ? (t1 - t0) : null,
        ms_aceptacion_a_cierre:    (t1 && t2) ? (t2 - t1) : null,
        ms_total:                  (t0 && t2) ? (t2 - t0) : null,
      };
    }
    const detalleConTiempos = detalleEnt.map(r => ({
      ...r, ...calcTiempos(r),
    }));
  ...
  app.get('/api/tareas/reportes.xlsx', auth, async (req, res) => {
    if (!X || !X.buildPolipayXLSX) return res.status(500).json({ error: 'excel_no_disponible' });
    const producto = req.query.producto ? String(req.query.producto) : null;
    const desde = req.query.desde ? String(req.query.desde) : null;
    const hasta = req.query.hasta ? String(req.query.hasta) : null;
    ...
    const fmtF = f => { if(!f) return '—'; const [y,m,dd]=f.split('-'); return `${dd} ${MESES[+m-1]} ${y}`; };
    const now = new Date();
    const stamp = `${String(now.getDate()).padStart(2,'0')}/${String(now.getMonth()+1).padStart(2,'0')}/${now.getFullYear()} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
    const meta = `${producto ? cap(producto) : 'Todos los productos'} · ${fmtF(desde)} → ${fmtF(hasta)} · Generado ${stamp}`;

    // Hoja 1 — Resumen (dos columnas: métrica, valor)
    const resumenRows = [
      ['RUTINAS Y TAREAS', ''],
    ...
    // Hoja 2 — Productividad por producto
    const prodRows = (d.productividad || []).map(p => [
      cap(p.producto),
      p.rutinas_total, p.rutinas_hechas, p.rutinas_en_curso, p.rutinas_vencidas,
      pct(p.rutinas_cumplimiento_pct),
      p.entregables_total, p.entregables_aprobadas, p.entregables_rechazadas,
      p.entregables_en_curso, p.entregables_vencidas,
      pct(p.sla_pct), pct(p.primera_vuelta_pct),
    ]);
    // Totales
    const totProd = (d.productividad || []).reduce((a,p) => ({
      rt:a.rt+p.rutinas_total, rh:a.rh+p.rutinas_hechas, rc:a.rc+p.rutinas_en_curso, rv:a.rv+p.rutinas_vencidas,
      et:a.et+p.entregables_total, ea:a.ea+p.entregables_aprobadas, er:a.er+p.entregables_rechazadas,
      ec:a.ec+p.entregables_en_curso, ev:a.ev+p.entregables_vencidas,
    }), {rt:0,rh:0,rc:0,rv:0,et:0,ea:0,er:0,ec:0,ev:0});
    const totRow = [
      'TOTAL',
      totProd.rt, totProd.rh, totProd.rc, totProd.rv,
      pct(totProd.rt > 0 ? Math.round((totProd.rh / totProd.rt) * 100) : null),
      totProd.et, totProd.ea, totProd.er, totProd.ec, totProd.ev,
      pct(d.entregables.sla_pct), pct(d.entregables.primera_vuelta_pct),
    ];

    // Formateo de duración: "2d 4h 15m" · "3h 20m" · "45m" · "—"
    const fmtDur = ms => {
    ...
    // Hoja 3 — Detalle por entregable con tiempos de transición
    const detRows = (d.detalle || []).map(r => [
      r.folio,
      (r.tipo_codigo || '') + (r.titulo ? ' · ' + r.titulo : ''),
      cap(r.producto || ''),
      r.estado || '',
      r.prioridad || '',
      ...
      fmtFH(r.creado_at),
      fmtFH(r.aceptada_at),
      fmtFH(r.fecha_cierre),
      fmtDur(r.ms_recepcion_a_aceptacion),
      fmtDur(r.ms_aceptacion_a_cierre),
      fmtDur(r.ms_total),
    ]);

    // Hoja 4 — Top ejecutores
    const topEjRows = (d.top_ejecutores || []).map(u => [u.nombre || '—', u.email || '—', u.n]);

    // Hoja 4 — Top áreas
    const topArRows = (d.top_areas || []).map(a => [a.area, a.n]);

    const buf = await X.buildPolipayXLSX({
      title: 'Reporte de operaciones — Productividad',
      meta,
      footer: 'Polipay POS Settlement · Notificación automática · Todas las métricas se calculan sobre el rango de fechas del filtro.',
      sheets: [
        {
          name: 'Resumen',
          columns: [
            { header: 'Métrica', width: 34 },
        ...
        },
        {
          name: 'Por producto',
          columns: [
            { header: 'Producto',         width: 16 },
            { header: 'Rutinas',          width: 10, align: 'right' },
            { header: 'Hechas',           width: 10, align: 'right' },
            { header: 'En curso',         width: 11, align: 'right' },
            { header: 'Vencidas',         width: 11, align: 'right' },
            { header: '% cumplimiento',   width: 15, align: 'right' },
            { header: 'Entregables',      width: 12, align: 'right' },
            { header: 'Aprobadas',        width: 11, align: 'right' },
            { header: 'Rechazadas',       width: 12, align: 'right' },
            { header: 'En curso',         width: 11, align: 'right' },
            { header: 'Vencidas',         width: 11, align: 'right' },
            { header: '% SLA',            width: 10, align: 'right' },
            { header: '% 1ª vuelta',      width: 12, align: 'right' },
          ],
          rows: prodRows.length ? prodRows : [['—','','','','','','','','','','','','']],
          total: prodRows.length ? totRow : null,
        },
        {
          name: 'Detalle por entregable',
          columns: [
            { header: 'Folio',              width: 18 },
            { header: 'Solicitud',          width: 42 },
            { header: 'Producto',           width: 14 },
            { header: 'Estado',             width: 14 },
            { header: 'Prioridad',          width: 11 },
            { header: 'Solicitante',        width: 22 },
            { header: 'Ejecutor',           width: 22 },
            { header: 'Recepción',          width: 17, align: 'center' },
            { header: 'Aceptación',         width: 17, align: 'center' },
            { header: 'Cierre',             width: 17, align: 'center' },
            { header: 'Recep. → Acept.',    width: 16, align: 'right' },
            { header: 'Acept. → Cierre',    width: 16, align: 'right' },
            { header: 'Total',              width: 14, align: 'right' },
          ],
          rows: detRows.length ? detRows : [['Sin entregables en el periodo','','','','','','','','','','','','']],
        },
        {
          name: 'Top ejecutores',
          columns: [
            ...
            { header: 'Entregables', width: 14, align: 'right' },
          ],
          rows: topEjRows.length ? topEjRows : [['Sin ejecutores en el periodo','','']],
        },
        {
          name: 'Áreas',
          columns: [
            { header: 'Área',         width: 32 },
            { header: 'Solicitudes',  width: 14, align: 'right' },
          ],
          rows: topArRows.length ? topArRows : [['Sin solicitudes en el periodo','']],
        },
      ],
    });
