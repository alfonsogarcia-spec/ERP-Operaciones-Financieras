...
let _avisoLlaveIncorrecta = false;
function decrypt(payload) {
  ...
  try {
    ...
    // Llave local no coincide con la que cifró el dato (ej. datos de prod
    // importados a un entorno con otra ENCRYPTION_KEY_V1) — no debe tumbar
    // el proceso por un solo campo no descifrable.
    if (!_avisoLlaveIncorrecta) {
      _avisoLlaveIncorrecta = true;
      console.warn('⚠ crypto.decrypt: no se pudo descifrar uno o más campos (llave incorrecta para este dato). Se devuelve null en esos campos.');
    }
    return null;
  }
}
