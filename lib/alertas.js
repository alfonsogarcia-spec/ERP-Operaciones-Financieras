...
const REGLAS = [
  ...
  {
    id: 'usuario_admin_cambio',
    titulo: 'Cambio en usuario con rol admin',
    // Alta, edición o baja de cualquier fila cuyo detalle indique rol=admin o cuya acción sea admin
    match: (f) => ['usuario_alta', 'usuario_editar', 'usuario_baja'].includes(f.accion) && /rol=admin|admin/.test(f.detalle || ''),
    ventana_min: 5,
    clave: (f) => f.resource_id || 'sin-id',
    umbral: async (_db, _f) => ({ n: 1 }),
    detalles: (f) => ({
      motivo: `Un administrador realizó un cambio en un usuario con rol admin.`,
      campos: [
        ['Acción', f.accion],
      ...
      sugerencia: 'Alta o cambio de admin es un evento de máxima sensibilidad. Confirma con el usuario administrador que realizó la acción que fue intencional.',
    }),
  },
  {
    id: 'corte_baja_final',
