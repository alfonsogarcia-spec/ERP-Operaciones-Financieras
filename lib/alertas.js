...
const REGLAS = [
  ...
  {
    id: 'disputa_cb_perdido',
    titulo: 'Contracargo perdido (débito al comercio)',
    // bit registra 'disputa_cb_status' con detalle 'X→LOST' cuando se pierde el caso.
    match: (f) => f.accion === 'disputa_cb_status' && /→LOST/.test(f.detalle || ''),
    ventana_min: 0,
    clave: (f) => f.resource_id || 'sin-id',
    umbral: async () => ({ n: 1 }),
    detalles: (f) => ({
      motivo: `Un contracargo terminó en estado LOST — el débito al comercio queda firme y no hay más recursos.`,
      campos: [
        ['Contracargo ID', f.resource_id || '—'],
        ['Transición', f.detalle || '—'],
        ['Usuario', f.usuario],
        ['IP', f.ip || '—'],
      ],
      sugerencia: 'Registra la razón del perdido en la bitácora del caso. Si el patrón se repite en un mismo comercio o reason code, revisa procesos de evidencia.',
    }),
  },
  {
    id: 'disputa_cb_arbitraje',
    titulo: 'Contracargo escaló a IN_DISPUTE / arbitraje',
    // bit registra 'disputa_cb_status' con detalle 'X→IN_DISPUTE' cuando se pasa a disputa activa.
    match: (f) => f.accion === 'disputa_cb_status' && /→IN_DISPUTE/.test(f.detalle || ''),
    ventana_min: 60,
    clave: (f) => f.resource_id || 'sin-id',
    umbral: async () => ({ n: 1 }),
    detalles: (f) => ({
      motivo: `Un contracargo entró en disputa activa (IN_DISPUTE). Puede escalar a arbitraje según la red.`,
      campos: [
        ['Contracargo ID', f.resource_id || '—'],
        ['Transición', f.detalle || '—'],
        ['Usuario', f.usuario],
      ...
      sugerencia: 'Prepara la evidencia final antes del cierre del ciclo. Revisa el timeline del caso y adjunta cualquier documento pendiente.',
    }),
  },
  {
    id: 'disputa_webhook_lote',
    titulo: 'Ingesta de contracargos por webhook (lote alto)',
    // ≥10 CB nuevos por webhook en 30 min (posible carga masiva del proveedor).
    match: (f) => f.accion === 'disputa_cb_webhook',
    ventana_min: 60,
    clave: () => 'global',
    umbral: async (db, _f) => {
      const r = await db.query("select count(*)::int as n from bitacora where accion='disputa_cb_webhook' and ts > now() - interval '30 minutes'");
      return r.rows[0].n >= 10 ? { n: r.rows[0].n } : null;
    },
    detalles: (_f, ctx) => ({
      motivo: `Se ingestaron ${ctx.n} contracargos automáticamente vía webhook en los últimos 30 minutos.`,
      campos: [['Ingresados en 30 min', ctx.n]],
      sugerencia: 'Revisa el listado en Disputas → Contracargos para validar que los casos son legítimos. Si es una carga masiva del proveedor, asegúrate de que los contactos activos estén al día para notificar a los comercios.',
    }),
  },
];
