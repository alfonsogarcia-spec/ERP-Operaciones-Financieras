...
/* ============================================================================
   PLANTILLA VISUAL POLIPAY (basada en el "Reporte por cliente" del corte).
   Todos los reportes .xlsx del sistema usan este helper para que compartan
   colores, tipografía, banda "POLIPAY" y pie. Escribe con ExcelJS (soporta
   fill / font / align / border / numFmt / merged / freeze).

   buildPolipayXLSX({
     title: 'Layout SPEI de dispersión',            // fila 2
     meta:  'Corte 42 · 04 de agosto de 2026 · Generado 06/08/2026',
     footer:'Polipay POS Settlement · Todas las cifras en MXN',
     sheets: [
       {
         name: 'LAYOUT',
         columns: [{ header, width, align, numFmt, bold }],
         rows: [[...]],
         total: [...]            // opcional
       },
       ...
   ...
   ========================================================================= */
async function buildPolipayXLSX(opts) {
  const ExcelJS = require('exceljs');
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Polipay POS Settlement';

  const AZUL_MARINO = 'FF051B3B';
  const AZUL_HEADER = 'FF3083F4';
  const GRIS_META   = 'FF667085';
  const NEGRO_TXT   = 'FF1A1A1A';
  const BLANCO      = 'FFFFFFFF';
  const BORDE_SUAVE = 'FFE4E6E7';
  const FONT_NAME   = 'Montserrat';
  const NUMFMT_MXN  = '"$"#,##0.00;[Red]("$"#,##0.00);-';
  const fill = c => ({ type: 'pattern', pattern: 'solid', fgColor: { argb: c } });

  for (const s of (opts.sheets || [])) {
    const nCols = (s.columns || []).length || 1;
    const ws = wb.addWorksheet((s.name || 'Hoja1').slice(0, 31), { views: [{ state: 'frozen', ySplit: 5 }] });
    // Ancho de columnas
    ws.columns = (s.columns || []).map(c => ({ width: c.width || 14 }));

    const setBand = (row, opts2) => {
      if (opts2.height) row.height = opts2.height;
      for (let col = 1; col <= nCols; col++) {
        const cell = row.getCell(col);
        if (opts2.fill) cell.fill = fill(opts2.fill);
        if (opts2.font) cell.font = Object.assign({ name: FONT_NAME }, opts2.font);
        if (opts2.align) cell.alignment = opts2.align;
      }
    };

    // Fila 1: banda POLIPAY (azul marino, texto blanco 20pt bold)
    const r1 = ws.addRow(['POLIPAY']); ws.mergeCells(1, 1, 1, nCols);
    setBand(r1, { height: 33.75, fill: AZUL_MARINO, font: { color: { argb: BLANCO }, bold: true, size: 20, name: FONT_NAME }, align: { horizontal: 'left', vertical: 'middle', indent: 1 } });

    // Fila 2: título en azul
    const r2 = ws.addRow([opts.title || '']); ws.mergeCells(2, 1, 2, nCols);
    setBand(r2, { height: 21.75, fill: BLANCO, font: { color: { argb: AZUL_HEADER }, bold: true, size: 12, name: FONT_NAME }, align: { horizontal: 'left', vertical: 'middle', indent: 1 } });

    // Fila 3: meta (fecha, corte, generado por…)
    const r3 = ws.addRow([opts.meta || '']); ws.mergeCells(3, 1, 3, nCols);
    setBand(r3, { height: 15.75, fill: BLANCO, font: { color: { argb: GRIS_META }, size: 9, name: FONT_NAME }, align: { horizontal: 'left', vertical: 'middle', indent: 1 } });

    // Fila 4: separador
    const r4 = ws.addRow(['']); ws.mergeCells(4, 1, 4, nCols);
    setBand(r4, { height: 3.75, fill: BLANCO });

    // Fila 5: encabezados
    const r5 = ws.addRow((s.columns || []).map(c => c.header || ''));
    r5.height = 30;
    for (let col = 1; col <= nCols; col++) {
      const cell = r5.getCell(col);
      const spec = s.columns[col - 1] || {};
      cell.fill = fill(AZUL_HEADER);
      cell.font = { name: FONT_NAME, color: { argb: BLANCO }, bold: true, size: 10 };
      const alignH = spec.align === 'left' ? 'left' : spec.align === 'right' ? 'right' : spec.align === 'center' ? 'center' : (spec.numFmt ? 'center' : 'left');
      cell.alignment = { horizontal: alignH, vertical: 'middle', indent: alignH === 'left' ? 1 : 0 };
    }

    // Filas de datos
    for (const dr of (s.rows || [])) {
      ...
      for (let col = 1; col <= nCols; col++) {
        ...
        const alignH = spec.align === 'left' ? 'left' : spec.align === 'right' ? 'right' : spec.align === 'center' ? 'center' : (spec.numFmt ? 'right' : 'left');
        cell.alignment = { horizontal: alignH, vertical: 'middle', indent: alignH === 'left' ? 1 : 0 };
        if (spec.numFmt) cell.numFmt = spec.numFmt === 'mxn' ? NUMFMT_MXN : spec.numFmt;
      }
    }

    // Fila TOTAL (opcional)
    if (Array.isArray(s.total) && s.total.length) {
      const rt = ws.addRow(s.total); rt.height = 22;
      for (let col = 1; col <= nCols; col++) {
        ...
        cell.alignment = { horizontal: alignH, vertical: 'middle', indent: alignH === 'left' ? 1 : 0 };
        if (spec.numFmt) cell.numFmt = spec.numFmt === 'mxn' ? NUMFMT_MXN : spec.numFmt;
        cell.border = { top: { style: 'thin', color: { argb: BORDE_SUAVE } } };
      }
    }
    ...
    // Pie
    if (opts.footer) {
      const rp = ws.addRow([opts.footer]);
      const pieRow = rp.number; ws.mergeCells(pieRow, 1, pieRow, nCols);
      setBand(rp, { height: 25.5, font: { color: { argb: GRIS_META }, size: 8, name: FONT_NAME }, align: { horizontal: 'left', vertical: 'top', indent: 1 } });
    }
  }
...
// Meta line comúnmente reutilizada por los reportes del corte.
function metaCorte(c) {
  ...
  return { fechaLarga, hoy };
}

module.exports = { TX_COLS, TX_ALIAS, pickCol, normHeader, aoaToObjects, parseBuffer, parseCSVText, buildXLSX, buildPolipayXLSX, metaCorte };
